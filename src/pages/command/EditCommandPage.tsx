...
export const EditCommandPage = component$(({ data }: { data?: Command }) => {
  ...
  const handleDelete = $(() => {
    ...
    nav(`${location.url.origin}/accounts/${data?.accountId}/commands`)
  })
    ...
    <Layout>
      ...
      >
        <FormControl>
          ...
          <Input name="name" id="command-name" value={data?.name} /></FormControl>   <FormControl>
          <label for="command-response">Command Response</label>
          <Input name="response" id="command-response" value={data?.response} /></FormControl>
        <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'center' }}>
          {data && <Button preventdefault:click onClick$={handleDelete} variant={ButtonVariant.SECONDARY}>Delete</Button>}
          <Button link={'../'} variant={ButtonVariant.SECONDARY}>{data ? 'Discard Changes' : 'Cancel'}</Button>
